...
const Signup = () => {
    ...
    const submitHandler = async (e) => {
        ...
        try {
            const { data } = await axios.post(`${import.meta.env.VITE_BASE_URL}/users/register`, newUser)
        ...
        <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
            <header className="absolute top-1 flex justify-between w-full items-center px-4">
               <Link to="/"> <img className="w-16" src={logo} alt="logo" /></Link>
                <Link to="/" className=" px-4 font-medium rounded-lg flex gap-2 py-[6px] bg-[linear-gradient(119.59deg,#3E0F77_22.24%,#FFB3B3_115.05%,#211331_135.87%)] text-white"><i className="ri-arrow-left-long-line"></i>Home</Link>
            </header>
            <div style={{
